...
zarr_node <- R6::R6Class('zarr_node',
  ...
  public = list(
    ...
    attribute = function(name) {
      super$attribute(private$.metadata[["attributes"]], name)
    },
    ...
    set_attribute = function(name, value) {
      private$.metadata[["attributes"]] <-
        super$set_attribute(private$.metadata[["attributes"]], name, value)
    ...
    delete_attribute = function(name) {
      atts <- super$delete_attribute(private$.metadata[["attributes"]], name)
      if (length(atts))
  ...
  active = list(
    ...
    #' @field dirty Flag to indicate that there are unsaved edits in the node.
    #'   Such edits always concern the metadata and then in particular the
    #'   user-level attributes. Setting the field to `TRUE` will ensure that the
    #'   edits are saved to the Zarr store.
    dirty = function(value) {
      if (missing(value))
        private$.meta_dirty
      else if (is.logical(value))
        private$.meta_dirty <- value[1L]
    },
    ...
    #'   and `append_array_attribute()` methods or removed with the
